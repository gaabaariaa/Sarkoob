...
GAME = Path('lib/screens/game_flow_screen.dart')
WORKFLOW = Path('.github/workflows/one-time-modern-intro-night.yml')
MARKER = 'VOTING_UI_V2'

s = GAME.read_text()
if MARKER not in s:
    start = s.index('  Widget _buildEliminationVoteSequence() {')
    end = s.index('  // ---------- رفراندومِ فعالِ مدنی', start)
    replacement = r'''  // VOTING_UI_V2 — ظاهرِ مدرنِ رأی‌گیری؛ منطقِ Controller دست‌نخورده است.
...
'''
    GAME.write_text(s[:start] + replacement + s[end:])
    print('VOTING_UI_V2 applied')
else:
    print('VOTING_UI_V2 already present')
