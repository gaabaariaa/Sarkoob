...
class HomeScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final primary = AppTheme.uiPrimary;
    final light = AppTheme.uiPrimaryLight;
    final dark = AppTheme.uiPrimaryDark;
    final bg = AppTheme.uiBackground;
    final card = AppTheme.uiCard;
    final surface = AppTheme.uiSurface;
    final elevated = AppTheme.uiElevated;
    final muted = AppTheme.uiMutedText;
    final subtle = AppTheme.uiSubtleText;
    return Scaffold(
      body: Stack(children: [
        Positioned.fill(child: DecoratedBox(decoration: BoxDecoration(color: bg, gradient: RadialGradient(center: const Alignment(0, -0.85), radius: 1.15, colors: [primary.withOpacity(.11), bg], stops: const [0, .72])), child: Stack(children: [Positioned(top: -90, right: -70, child: _GlowOrb(size: 220, color: primary, opacity: .045)), Positioned(bottom: 40, left: -100, child: _GlowOrb(size: 260, color: primary, opacity: .025))]))),
        SafeArea(child: LayoutBuilder(builder: (context, constraints) {
          ...
          return SingleChildScrollView(physics: const BouncingScrollPhysics(), padding: EdgeInsets.fromLTRB(wide ? 40 : 18, 18, wide ? 40 : 18, 28), child: Center(child: ConstrainedBox(constraints: const BoxConstraints(maxWidth: 920), child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            ...
            _HeroPanel(primary: primary, light: light, surface: surface, card: card, muted: muted), const SizedBox(height: 20),
...
class _GlowOrb extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => ImageFiltered(imageFilter: ImageFilter.blur(sigmaX: 45, sigmaY: 45), child: Container(width: size, height: size, decoration: BoxDecoration(shape: BoxShape.circle, color: color.withOpacity(opacity))));
}
...
class _TopBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(children: [
    Container(width: 44, height: 44, decoration: BoxDecoration(borderRadius: BorderRadius.circular(14), color: card, border: Border.all(color: primary.withOpacity(.28))), child: Icon(Icons.auto_awesome, color: light, size: 22)), const SizedBox(width: 12),
    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('دست خدا', style: AppTheme.headingFont(size: 25, color: light)), Text('دستیار گرداننده بازی نقش مخفی', style: Theme.of(context).textTheme.bodySmall?.copyWith(color: muted))])),
    Container(padding: const EdgeInsets.symmetric(horizontal: 11, vertical: 7), decoration: BoxDecoration(color: primary.withOpacity(.08), borderRadius: BorderRadius.circular(20), border: Border.all(color: primary.withOpacity(.2))), child: Row(mainAxisSize: MainAxisSize.min, children: [Container(width: 6, height: 6, decoration: BoxDecoration(color: primary, shape: BoxShape.circle)), const SizedBox(width: 6), Text('آماده', style: Theme.of(context).textTheme.labelMedium?.copyWith(color: light, fontWeight: FontWeight.w700))]))
  ]);
}

class _HeroPanel extends StatelessWidget {
  final Color primary, light, surface, card, muted;
  const _HeroPanel({required this.primary, required this.light, required this.surface, required this.card, required this.muted});
  @override
  Widget build(BuildContext context) => Container(padding: const EdgeInsets.fromLTRB(22, 25, 22, 22), decoration: BoxDecoration(borderRadius: BorderRadius.circular(26), gradient: LinearGradient(begin: Alignment.topRight, end: Alignment.bottomLeft, colors: [AppTheme.uiElevated, surface]), border: Border.all(color: primary.withOpacity(.24)), boxShadow: [BoxShadow(color: Colors.black.withOpacity(.35), blurRadius: 24, offset: const Offset(0, 12))]), child: Column(children: [
    Container(width: 76, height: 76, decoration: BoxDecoration(shape: BoxShape.circle, gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [light, primary]), boxShadow: [BoxShadow(color: primary.withOpacity(.18), blurRadius: 26, spreadRadius: 2)]), child: const Icon(Icons.theater_comedy_rounded, color: Color(0xFF151515), size: 38)), const SizedBox(height: 16),
    Text('میز بازی', style: AppTheme.headingFont(size: 31, color: light)), const SizedBox(height: 4), Text('همه‌چیز برای اجرای یک شب پرتنش آماده است.', textAlign: TextAlign.center, style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: muted, height: 1.7)), const SizedBox(height: 17),
    Row(mainAxisAlignment: MainAxisAlignment.center, children: [_HeroTag(icon: Icons.shield_outlined, text: 'نقش مخفی', primary: primary, light: light), const SizedBox(width: 8), _HeroTag(icon: Icons.nights_stay_outlined, text: 'سناریوی سرکوب', primary: primary, light: light)])
  ]));
}
...
class _StartButton extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Game3DSurface(onPressed: onTap, customColors: Game3DColors.fromColor(primary), depth: 6, borderRadius: BorderRadius.circular(19), padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 18), semanticLabel: 'شروع بازی', child: Row(children: [
    Container(width: 48, height: 48, decoration: BoxDecoration(color: Colors.black.withOpacity(.12), borderRadius: BorderRadius.circular(15)), child: Icon(Icons.play_arrow_rounded, size: 31, color: light)), const SizedBox(width: 14),
    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('شروع بازی', style: AppTheme.headingFont(size: 23, color: light)), Text('بازیکنان را انتخاب کن و وارد سناریو شو', style: TextStyle(color: light.withOpacity(.72), fontWeight: FontWeight.w600, fontSize: 12))])), Icon(Icons.arrow_back_rounded, color: light)
  ]));
}
...
class _SettingsButton extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Game3DSurface(onPressed: onTap, customColors: Game3DColors.fromColor(AppTheme.uiCard), depth: 5, borderRadius: BorderRadius.circular(19), padding: const EdgeInsets.symmetric(horizontal: 18, vertical: 16), semanticLabel: 'تنظیمات', child: Row(children: [
...
class _SectionTitle extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Row(children: [Text(title, style: Theme.of(context).textTheme.titleMedium?.copyWith(color: AppTheme.uiPrimaryLight, fontWeight: FontWeight.w800)), const SizedBox(width: 10), Expanded(child: Divider(color: AppTheme.uiPrimary.withOpacity(.13)))]);
}
...
class _HomeTile extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Game3DSurface(onPressed: onTap, customColors: Game3DColors.fromColor(card), depth: 4, borderRadius: BorderRadius.circular(17), padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 12), semanticLabel: title, child: Row(children: [
    Container(width: 42, height: 42, decoration: BoxDecoration(color: primary.withOpacity(.09), borderRadius: BorderRadius.circular(13), border: Border.all(color: primary.withOpacity(.16))), child: Icon(icon, color: primary, size: 22)), const SizedBox(width: 10),
